...
const StatCard = ({ title, value, icon, color, onClick, clickable = false }: StatCardProps) => (
  <div
    className={`card text-white mb-3 bg-${color} shadow-sm ${clickable ? 'cursor-pointer card-hover' : ''
      }`}
    onClick={onClick}
...
const DashboardStats = () => {
  ...
  return (
    <div>
      ...
      {/* Enlace a Más Vendidos */}
      <div className="card text-white mb-3 bg-primary shadow-sm cursor-pointer card-hover"
        ...
        <div className="card-body d-flex align-items-center">
          <TrendingDown size={30} className="text-white" style={{ transform: 'rotate(180deg)' }} /> {/* Using TrendingDown rotated as TrendingUp alternative or import TrendingUp */}
          <div className='ms-3'>
            <h5 className="card-title mb-1">🏆 Productos Más Vendidos</h5>
            <p className="card-text mb-0 small">
              Ver ranking detallado y filtros 👆
            </p>
          </div>
        </div>
      </div>
